...
import numpy as np
from ..simulations.collatz_simulations import collatz_sequence

def plot_3d_trajectory(n, save_path=None):
    seq = collatz_sequence(n)
    
    # Convertir secuencia a coordenadas 3D
    x = [np.log(s) for s in seq]  # Logaritmo del valor
    y = list(range(len(seq)))     # Paso de la secuencia
    z = [s % 10 for s in seq]     # Valor módulo 10 (para dimensión adicional)
